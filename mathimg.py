...
def make_img(txt: str, format: str = 'png', fileName: str = 'output', customDpi: int = 300, color: str = 'White'):
    mpl.use('Agg') # Set backend to Agg
    fig = figure.Figure(figsize=(4, 2))
    FigureCanvas(fig)
    fig.text(0, 0, txt, color=color, fontsize=texFont.get_size_in_points(), fontproperties=texFont)
    fileName += f'.{format}'
    fig.savefig(fileName, dpi=customDpi, transparent=True, bbox_inches='tight', pad_inches=0)
    return fileName
...
def render_output(new_txt: list | str, fileName='output'):
    ...
    return make_img(txt, 'png', fileName, 300, 'White')
