    ...
    for char in text:
        if char == "$":
            if in_control_sequence:
                # Adjust line length by the visual length of the control sequence
        ...
        if in_control_sequence:
            ...
            if line_length >= width:
                ...
                if last_word_match:
                    ...
                    # Check if the last word can fit on the next line
                    if word_length > width:
                        # If the word itself is longer than the width, split it across lines
                        lines.append(current_line.rstrip())
                        current_line = last_word
                    else:
                        # Move the last word to the next line if it doesn't fit
